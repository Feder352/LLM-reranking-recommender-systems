...
TITLE_CANDIDATES = ["title", "item_title", "product_title", "movie_title", "name", "item_name"]
...
def main():
    ...
    ap.add_argument("--inter_path", default="Amazon_Books_cut500.inter")
    ap.add_argument("--item_path", default="Amazon_Books_cut500.item")
    ap.add_argument("--user_path", default="Amazon_Books_cut500.user")
    ap.add_argument("--lightgcn_recs_path", default="LightGCN_top50_seed2020.csv")
    ap.add_argument("--ckpt_path", default="LightGCN-Jan-27-2026_18-16-05.pth")
    ...
    ap.add_argument("--min_pos_rating", type=float, default=None)
    ...
    lightgcn = pd.read_csv(args.lightgcn_recs_path)
    ...
    if not needed_user.issubset(users.columns):
        raise ValueError(f"{args.user_path} colonne mancanti: {list(users.columns)}")
    if not needed_recs.issubset(lightgcn.columns):
        raise ValueError(f"{args.lightgcn_recs_path} colonne mancanti: {list(lightgcn.columns)}")

    title_col = args.title_col.strip() or find_first_existing(items.columns, TITLE_CANDIDATES, required=True, what="la colonna titolo")
    ...
    user_token_to_hist_items = {uid: df["item_id"].astype(str).tolist() for uid, df in inter_hist.groupby("user_id")}

    user_to_recs = {}
    bad_len = []
    for _, row in lightgcn.iterrows():
    ...
    if bad_len:
        raise RuntimeError(f"Alcuni utenti hanno meno di {args.k_candidates} raccomandazioni. Esempi: {bad_len[:10]}")
                ...
                if iid_inner >= len(inner2item):
                    ...
                    if item_token == "[PAD]":
                        ...
                        if title is None or title == "nan":
                            ...
                            title = f"ITEM_{item_token}"
                candidates.append({"rank": rank, "item_id": int(iid_inner), "item_name": title})
