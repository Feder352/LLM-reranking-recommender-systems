...
PROMPTS_DIR = Path("amazon_lightgcn_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_amazon_lightgcn_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_amazon_lightgcn_rerank.jsonl")
LIGHTGCN_RECS_FILE = Path("LightGCN_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_amazon_lightgcn_top10_recbole_like.csv")
...
def extract_candidates_from_prompt(prompt_text: str) -> List[dict]:
    ...
    names = re.findall(r'\{\s*"rank"\s*:\s*(\d+)\s*,\s*"item_id"\s*:\s*(\d+)\s*,\s*"item_name"\s*:\s*"((?:[^"\\]|\\.)*)"\s*\}', prompt_text)
    out = []
...
def find_output_file_name(job_dict: dict) -> str:
    for a, b in [("dest", "fileName"), ("dest", "file_name"), ("output", "fileName"), ("output", "file_name"), ("output_file", "fileName"), ("output_file", "file_name"), ("result", "fileName"), ("result", "file_name")]:
...
def load_lightgcn_recs(path: Path) -> Dict[int, List[int]]:
    df = pd.read_csv(path)
    needed = {"user_id", "recs"}
...
def run_gemini_batch(client, prompt_map: Dict[str, Path]) -> None:
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(model=model_name, src=uploaded.name, config={"display_name": f"amazon-lightgcn-{len(prompt_map)}-users-rerank"})
...
def main():
    for p, label in [(PROMPTS_DIR, "Cartella prompt"), (LIGHTGCN_RECS_FILE, "LightGCN_top50_seed2020.csv")]:
        if not p.exists():
    ...
    print("Carico LightGCN top50...")
    lightgcn_user_to_recs = load_lightgcn_recs(LIGHTGCN_RECS_FILE)
    print(f"Utenti nel LightGCN csv: {len(lightgcn_user_to_recs)}")
    ...
    with OUT_RECS_CSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for uid_str in sorted(prompt_map.keys(), key=lambda x: int(x)):
            ...
            lightgcn_item_ids = lightgcn_user_to_recs.get(uid_inner, [])
            if not lightgcn_item_ids:
                skipped_users += 1
                continue

            candidate_ids = prompt_candidate_ids if prompt_candidate_ids else lightgcn_item_ids
            candidate_set = set(candidate_ids)
            fallback_source = lightgcn_item_ids
    ...
    print("\nCSV compatibile con eval_definitivo_amazon_lightgcn.py (inner RecBole IDs)")
    print("Nessun item fuori dalla top50 LightGCN può entrare nel CSV finale.")
