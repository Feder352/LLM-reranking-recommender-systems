...
PROMPTS_DIR = Path("amazon_cke_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_amazon_cke_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_amazon_cke_rerank.jsonl")
CKE_RECS_FILE = Path("CKE_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_amazon_cke_top10_recbole_like.csv")
...
def load_cke_recs(path: Path) -> Dict[int, List[int]]:
    df = pd.read_csv(path)
    needed = {"user_id", "recs"}
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(
                model=model_name,
                src=uploaded.name,
                config={"display_name": f"amazon-cke-{len(prompt_map)}-users-rerank"},
            )
...
def main():
    for p, label in [(PROMPTS_DIR, "Cartella prompt"), (CKE_RECS_FILE, "CKE_top50_seed2020.csv")]:
        if not p.exists():
    ...
    print("Carico CKE top50...")
    cke_user_to_recs = load_cke_recs(CKE_RECS_FILE)
    print(f"Utenti nel CKE csv: {len(cke_user_to_recs)}")
    ...
    with OUT_RECS_CSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for uid_str in sorted(prompt_map.keys(), key=lambda x: int(x)):
            ...
            cke_item_ids = cke_user_to_recs.get(uid_inner, [])
            if not cke_item_ids:
                skipped_users += 1
                continue

            candidate_ids = prompt_candidate_ids if prompt_candidate_ids else cke_item_ids
            candidate_set = set(candidate_ids)
            fallback_source = cke_item_ids
    ...
    print("\nCSV compatibile con eval_definitivo_amazon_cke.py (inner RecBole IDs)")
    print("Nessun item fuori dalla top50 CKE può entrare nel CSV finale.")
