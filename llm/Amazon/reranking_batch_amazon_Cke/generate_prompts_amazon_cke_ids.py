...
TITLE_CANDIDATES = ["title", "item_title", "item_name", "book_title", "name", "product_title"]
...
def main():
    ...
    # History dal dataset KG Amazon
    ap.add_argument("--inter_path", default="Amazon-book-KG-500.inter")
    # Titoli dal dataset Amazon standard
    ap.add_argument("--item_path", default="Amazon_Books_cut500.item")
    ap.add_argument("--cke_recs_path", default="CKE_top50_seed2020.csv")
    ap.add_argument("--ckpt_path", default="CKE-Jan-27-2026_21-12-15.pth")
    ...
    cke = pd.read_csv(args.cke_recs_path)
    ...
    if not needed_recs.issubset(cke.columns):
        raise ValueError(f"{args.cke_recs_path} colonne mancanti: {list(cke.columns)}")
