...
def load_recbole_mappings(ckpt_path: str):
    """
    Carica dataset/mapping dal checkpoint RecBole.
    Serve perché BPR_top50_seed2020.csv contiene inner ids, mentre i file Amazon hanno token originali.
    """
    ...
    return {
        ...
        "uid_field": uid_field,
        "iid_field": iid_field,
        "inner2user": inner2user,
        "inner2item": inner2item,
    }
...
def main():
    ...
    # ===== FILE GIUSTI DEL TUO DATASET =====
    ap.add_argument("--inter_path", default="Amazon_Books_cut500.inter", help="Amazon_Books_cut500.inter")
    ap.add_argument("--item_path", default="Amazon_Books_cut500.item", help="Amazon_Books_cut500.item")
    ap.add_argument("--user_path", default="Amazon_Books_cut500.user", help="Amazon_Books_cut500.user")
    ap.add_argument("--bpr_recs_path", default="BPR_top50_seed2020.csv", help="BPR_top50_seed2020.csv")
    ap.add_argument("--ckpt_path", required=True, help="Checkpoint RecBole .pth coerente con BPR_top50_seed2020.csv")
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Soglia positivi. Se il dataset ha rating, tiene solo rating >= soglia. Se assente o None, usa tutte le interazioni.",
    )
    ap.add_argument(
        "--title_col",
        default="",
        help="Nome colonna titolo item da usare. Se vuoto, prova a rilevarla automaticamente.",
    )
    ...
    if not needed_inter.issubset(inter.columns):
        raise ValueError(f"{args.inter_path} colonne mancanti. Trovate: {list(inter.columns)}")
    if not needed_item.issubset(items.columns):
        raise ValueError(f"{args.item_path} colonne mancanti. Trovate: {list(items.columns)}")
    if not needed_user.issubset(users.columns):
        raise ValueError(f"{args.user_path} colonne mancanti. Trovate: {list(users.columns)}")
    if not needed_bpr.issubset(bpr.columns):
        raise ValueError(f"{args.bpr_recs_path} colonne mancanti. Trovate: {list(bpr.columns)}")
    ...
    # item token originale -> titolo
    items["item_id"] = items["item_id"].astype(str)
    item_token_to_title = dict(zip(items["item_id"], items[title_col].astype(str)))

    # user token originale -> lista cronologica item token originali
    ...
    if bad_len:
        raise RuntimeError(
            f"ERRORE: alcuni utenti hanno meno di {args.k_candidates} raccomandazioni nel CSV. Esempi: {bad_len[:10]}"
        )
    ...
    try:
        for uid_inner in sorted(user_to_recs.keys()):
            # user inner -> token originale
            if uid_inner >= len(inner2user):
                missing_user_mapping += 1
                print(f"[WARN] user inner id {uid_inner} fuori mapping checkpoint, skip.")
                continue

            user_token = inner2user[uid_inner]
            if user_token == "[PAD]":
                missing_user_mapping += 1
                print(f"[WARN] user inner id {uid_inner} mappa a [PAD], skip.")
                continue

            # history utente (token originali item)
            hist_item_tokens = user_token_to_hist_items.get(str(user_token), [])
            if not hist_item_tokens:
                empty_hist += 1

            hist_item_tokens = hist_item_tokens[-args.max_hist:]
            hist_titles = []

            for item_token in hist_item_tokens:
            ...
            # candidati: inner item -> token item -> title
            rec_inner_ids = user_to_recs[uid_inner][: args.k_candidates]
            cand_titles = []

            for iid_inner in rec_inner_ids:
            ...
            # salvo col nome dell'user INNER id, così resta coerente col BPR e col reranking successivo
