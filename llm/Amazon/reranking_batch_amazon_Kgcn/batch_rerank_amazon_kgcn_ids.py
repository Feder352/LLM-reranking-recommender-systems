...
PROMPTS_DIR = Path("amazon_kgcn_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_amazon_kgcn_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_amazon_kgcn_rerank.jsonl")
KGCN_RECS_FILE = Path("KGCN_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_amazon_kgcn_top10_recbole_like.csv")
...
def load_kgcn_recs(path: Path) -> Dict[int, List[int]]:
    df = pd.read_csv(path)
    needed = {"user_id", "recs"}
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(
                model=model_name,
                src=uploaded.name,
                config={"display_name": f"amazon-kgcn-{len(prompt_map)}-users-rerank"},
            )
...
def main():
    for p, label in [(PROMPTS_DIR, "Cartella prompt"), (KGCN_RECS_FILE, "KGCN_top50_seed2020.csv")]:
        if not p.exists():
    ...
    print("Carico KGCN top50...")
    kgcn_user_to_recs = load_kgcn_recs(KGCN_RECS_FILE)
    print(f"Utenti nel KGCN csv: {len(kgcn_user_to_recs)}")
    ...
    with OUT_RECS_CSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for uid_str in sorted(prompt_map.keys(), key=lambda x: int(x)):
            ...
            kgcn_item_ids = kgcn_user_to_recs.get(uid_inner, [])
            if not kgcn_item_ids:
                skipped_users += 1
                continue

            candidate_ids = prompt_candidate_ids if prompt_candidate_ids else kgcn_item_ids
            candidate_set = set(candidate_ids)
            fallback_source = kgcn_item_ids
    ...
    print("\nCSV compatibile con eval_definitivo_amazon_kgcn.py (inner RecBole IDs)")
    print("Nessun item fuori dalla top50 KGCN può entrare nel CSV finale.")
