...
PROMPTS_DIR = Path("amazon_dmf_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_amazon_dmf_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_amazon_dmf_rerank.jsonl")
DMF_RECS_FILE = Path("DMF_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_amazon_dmf_top10_recbole_like.csv")
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(
                model=model_name,
                src=uploaded.name,
                config={"display_name": f"amazon-dmf-{len(prompt_map)}-users-rerank"},
            )
...
def main():
    ...
    print("\nCSV compatibile con eval_definitivo_amazon_dmf.py (inner RecBole IDs)")
