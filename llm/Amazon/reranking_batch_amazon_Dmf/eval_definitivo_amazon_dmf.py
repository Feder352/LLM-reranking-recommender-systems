"""
Valutazione baseline e reranked per Amazon Books cut500 con DMF.
...
- include una diagnostica sulla top50 DMF per controllare coerenza col checkpoint
- per DMF, le metriche Yan/Unexpectedness usano i vettori finali costruiti dal modello
"""
...
CKPT_PATH = Path("DMF-Jan-27-2026_18-03-10.pth")
TOP50_CSV = Path("DMF_top50_seed2020.csv")
RERANKED_CSV = Path("reranked_amazon_dmf_top10_recbole_like.csv")
TOPKS = [5, 10]
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUT_CSV = Path("metrics_amazon_dmf.csv")
...
def get_embeddings(model, dataset, device):
    """
    Estrae embedding utente/item confrontabili nello stesso spazio.
    Per DMF usa la pipeline interna del modello per costruire:
      - user embedding finali
      - item embedding finali
    """
    ...
    if name == "dmf":
        try:
            ...
            with torch.no_grad():
                ...
                # ---- user vectors ----
                for start in range(0, dataset.user_num, batch_size):
                    end = min(start + batch_size, dataset.user_num)
                    users = torch.arange(start, end, device=device)
                    emb = model.get_user_embedding(users)
                    emb = model.user_fc_layers(emb)
                    u_list.append(emb.detach().cpu())

                # ---- item vectors ----
                for start in range(0, dataset.item_num, batch_size):
                    end = min(start + batch_size, dataset.item_num)
                    items = torch.arange(start, end, device=device)

                    col_indices = model.history_user_id[items].flatten()
                    row_indices = torch.arange(items.shape[0], device=device).repeat_interleave(
                        model.history_user_id.shape[1], dim=0
                    )

                    matrix_01 = torch.zeros(items.shape[0], model.n_users, device=device)
                    matrix_01.index_put_(
                        (row_indices, col_indices),
                        model.history_user_value[items].flatten()
                    )

                    emb = model.item_linear(matrix_01)
                    emb = model.item_fc_layers(emb)
                    i_list.append(emb.detach().cpu())
            ...
            print(f"ATTENZIONE: impossibile costruire embedding DMF: {e}")
            return None, None

    # fallback generico per altri modelli
    def extract(obj):
    ...
    for a in ["item_embedding", "item_embeddings", "item_embedding_layer"]:
        if hasattr(model, a):
...
def main():
    ...
    print("\n" + "=" * 60)
    print("STEP 2b — DIAGNOSTICA top50 DMF")
    print("=" * 60)
    ...
    u_embs, i_embs = get_embeddings(model, train_data.dataset, DEVICE)
