...
eval_definitivo.py
==================
Script DEFINITIVO per valutare baseline e reranked in modo coerente.

PROBLEMA risolto:
  Il ground truth di RecBole (positive_u/positive_i dal FullSortEvalDataLoader)
  è diverso da inter_feat completo del test set. Tutti gli script precedenti
  costruivano la ground truth sbagliata, causando hits e pos_len errati.

SOLUZIONE:
  1. Baseline: usa Trainer.evaluate() direttamente -> metriche identiche al benchmark
  2. Reranked: intercetta positive_u/positive_i dal loop interno di RecBole
     per costruire la ground truth CORRETTA, poi valuta il CSV reranked con quella.

FIX AGGIUNTIVO:
  Trainer.evaluate() con metriche come averagepopularity richiede che il collector
  abbia registrato le statistiche del train set tramite:
      trainer.eval_collector.data_collect(train_data)

COME USARE:
  python eval_definitivo.py
"""
...
RERANKED_CSV = Path("reranked_recbole_like.csv")
TOPKS        = [5, 10]
DEVICE       = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUT_CSV      = Path("metrics_definitivo.csv")
...
def extract_ground_truth_from_recbole(test_data, model, dataset, device):
    ...
    per calcolare le metriche. Questo garantisce coerenza col benchmark.

    Restituisce:
      gt: dict[uid_interno] -> set di item_id interni (ground truth)
      n_items: numero totale di item nel dataset
    """
    uid_field = dataset.uid_field
    n_items   = dataset.item_num

    gt = defaultdict(set)  # uid_interno -> set(item_id interni)
    ...
    with torch.no_grad():
        for batched_data in test_data:
            ...
            # positive_u è un indice di riga relativo al batch
...
def evaluate_baseline(config, model, train_data, test_data, topks, metrics):
    ...
    IMPORTANTE:
    - config["topk"] e config["metrics"] vanno impostati su una COPIA del config
    - per metriche come averagepopularity bisogna registrare le statistiche
      del train set nel collector con data_collect(train_data)
    """
...
def build_datastruct_from_csv(user_recs: dict, ground_truth: dict,
    ...
    (estratta con extract_ground_truth_from_recbole).
...
def main():
    ...
    print("\nBenchmark atteso (BPR ml-1m):")
    print("  ndcg@5=0.2465  ndcg@10=0.2369")
    print("  recall@5=0.0903  recall@10=0.1476")
    print("  precision@5=0.2230  precision@10=0.1887")
    ...
    # baseline recommendations costruite nello stesso modo del full-sort
