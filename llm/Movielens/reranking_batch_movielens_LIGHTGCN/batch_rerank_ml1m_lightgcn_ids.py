...
PROMPTS_DIR = Path("ml1m_lightgcn_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_ml1m_lightgcn_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_ml1m_lightgcn_rerank.jsonl")
LIGHTGCN_RECS_FILE = Path("LightGCN_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_ml1m_lightgcn_top10_recbole_like.csv")
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(
                model=model_name,
                src=uploaded.name,
                config={"display_name": f"ml1m-lightgcn-{len(prompt_map)}-users-rerank"}
            )
...
def main():
    ...
    print("\nCSV compatibile con eval_definitivo_ml1m_lightgcn.py (inner RecBole IDs)")
