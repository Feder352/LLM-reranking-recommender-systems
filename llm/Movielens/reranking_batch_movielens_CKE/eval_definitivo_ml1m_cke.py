"""
Valutazione baseline e reranked per MovieLens 1M con CKE.
...
- include una diagnostica sulla top50 CKE per controllare coerenza col checkpoint
"""
...
CKPT_PATH = Path("CKE-Jan-29-2026_14-50-12.pth")
TOP50_CSV = Path("CKE_top50_seed2020.csv")
RERANKED_CSV = Path("reranked_ml1m_cke_top10_recbole_like.csv")
TOPKS = [5, 10]
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUT_CSV = Path("metrics_ml1m_cke.csv")
...
def evaluate_baseline(config, model, train_data, test_data, topks, metrics):
    ...
    # compatibilità con dataloader standard e knowledge-based
    train_dataset = get_train_dataset(train_data)

    # Popoliamo manualmente i campi richiesti dalle metriche RecBole
    trainer.eval_collector.data_struct.set("data.num_items", train_dataset.item_num)
    trainer.eval_collector.data_struct.set("data.num_users", train_dataset.user_num)
    trainer.eval_collector.data_struct.set("data.count_items", train_dataset.item_counter)
    trainer.eval_collector.data_struct.set("data.count_users", train_dataset.user_counter)
...
def get_embeddings(model):
    """
    Estrae embedding confrontabili per CKE.
    Per CKE spesso esistono embedding utente/item e embedding entity.
    Privilegiamo item_embedding; fallback su entity_embedding.
    """
    def extract(obj):
    ...
    for a in ["item_embedding", "item_embeddings", "item_embedding_layer", "entity_embedding", "entity_embedding_layer"]:
        if hasattr(model, a):
...
def main():
    ...
    print("\n" + "=" * 60)
    print("STEP 2b — DIAGNOSTICA top50 CKE")
    print("=" * 60)
