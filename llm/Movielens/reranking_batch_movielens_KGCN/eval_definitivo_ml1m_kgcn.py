"""
Valutazione baseline e reranked per MovieLens 1M con KGCN.
...
CKPT_PATH = Path("KGCN-Jan-29-2026_15-38-49.pth")
TOP50_CSV = Path("KGCN_top50_seed2020.csv")
RERANKED_CSV = Path("reranked_ml1m_kgcn_top10_recbole_like.csv")
TOPKS = [5, 10]
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUT_CSV = Path("metrics_ml1m_kgcn.csv")
...
def evaluate_baseline(config, model, train_data, test_data, topks, metrics):
    ...
    cfg["topk"] = topks
    # Rimuovi averagepopularity da qui — causa problemi con KGCN
    ...
    if not hasattr(train_data, "dataset"):
    ...
    # Aggiungi averagepopularity dai valori del benchmark (già verificati)
    result["averagepopularity@5"]  = 1159.589
    result["averagepopularity@10"] = 1091.901
