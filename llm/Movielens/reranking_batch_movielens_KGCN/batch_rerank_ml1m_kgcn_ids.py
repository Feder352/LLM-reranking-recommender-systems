...
PROMPTS_DIR = Path("ml1m_kgcn_prompts")
PROMPT_GLOB = "prompt_user_*.txt"
TOPK = 10
BATCH_INPUT_FILE = Path("batch_requests_ml1m_kgcn_rerank.jsonl")
BATCH_RESULTS_JSONL = Path("batch_results_ml1m_kgcn_rerank.jsonl")
KGCN_RECS_FILE = Path("KGCN_top50_seed2020.csv")
OUT_RECS_CSV = Path("reranked_ml1m_kgcn_top10_recbole_like.csv")
    ...
    for model_name in MODEL_CANDIDATES:
        try:
            ...
            batch_job = client.batches.create(
                model=model_name,
                src=uploaded.name,
                config={"display_name": f"ml1m-kgcn-{len(prompt_map)}-users-rerank"},
            )
...
def main():
    ...
    print("\nCSV compatibile con eval_definitivo_ml1m_kgcn.py (inner RecBole IDs)")
