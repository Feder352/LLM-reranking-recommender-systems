...
def save_recs_multiple_k(dataset_name, data_path, model_name, seed, uid_field, iid_field, user_recs, ks, u_embs=None, i_embs=None):
    saved = []
    for k in ks:
        # taglia le recs al top-k richiesto
        user_recs_k = {u: r[:k] for u, r in user_recs.items()}
        global SAVE_TOPK
        old = SAVE_TOPK
        SAVE_TOPK = k
        rec_path, meta_path = save_recs(dataset_name, data_path, model_name, seed, uid_field, iid_field, user_recs_k, u_embs=u_embs, i_embs=i_embs)
        SAVE_TOPK = old
